...
public class AlunoMapper {
    ...
    public AlunoResponse toResponse(Aluno aluno) {
        List<MatriculaDTO> matriculasDTOS = aluno.getMatriculas().stream().map( m ->
             new MatriculaDTO(m.getCodigoMatricula(),m.getNomeCurso(),m.getDataInicio())).toList();
        return new AlunoResponse(aluno.getId(), aluno.getNome(), aluno.getDataNascimento(),matriculasDTOS);
    }
